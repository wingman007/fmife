...
//	mongoose.connect('mongodb://node:node@mongo.onmodulus.net:27017/uwO3mypu'); 	// connect to mongoDB database on modulus.io
	...
	app.post('/api/phoneBook', function(req, res) {
		...
		PhoneBook.create({
			...
			//find accepts a function as a callback function with which it returns the entries found with a json format
	...
	app.put('/api/phoneBook/:PhoneBook_id', function(req, res) {
		PhoneBook.update({ _id: req.body._id },{Fname: req.body.Fname, Lname: req.body.Lname, Number: req.body.Number},
		function(err){
				if (err){
					res.send(err);
				}
